...
if __name__ == "__main__":
    ...
    mle_losses = run(real_world, args.n_leaves, args.n_models, moment_based=False)
    # MOMENT BASED
    np.random.seed(args.seed)
    moment_based_losses = run(real_world, args.n_leaves, args.n_models, moment_based=True)

    np.save(f"data/mle_{args.seed}.npy", mle_losses)
    np.save(f"data/moment_based_{args.seed}.npy", moment_based_losses)